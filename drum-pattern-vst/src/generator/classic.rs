...
fn pattern_amen() -> Pattern {
    ...
    for &s in &[0, 3, 6, 10] { p.steps[s].instruments[0] = true; }
    for &s in &[4, 7, 12, 14] { p.steps[s].instruments[1] = true; }
    for &s in &[0, 2, 4, 6, 8, 10, 12, 14] { p.steps[s].instruments[2] = true; }
    p
}
...
fn pattern_funky_drummer() -> Pattern {
    ...
    for &s in &[0, 5, 8, 11] { p.steps[s].instruments[0] = true; }
    for &s in &[4, 7, 12, 15] { p.steps[s].instruments[1] = true; }
    for s in 0..STEP_COUNT { p.steps[s].instruments[2] = true; }
    p
}
...
fn pattern_bossa_nova() -> Pattern {
    ...
    for &s in &[0, 9] { p.steps[s].instruments[0] = true; }
    for &s in &[4, 12] { p.steps[s].instruments[1] = true; }
    for &s in &[0, 2, 4, 6, 8, 10, 12, 14] { p.steps[s].instruments[2] = true; }
    p
}
...
fn pattern_motorik() -> Pattern {
    ...
    for &s in &[0, 4, 8, 12] { p.steps[s].instruments[0] = true; }
    for &s in &[4, 12] { p.steps[s].instruments[1] = true; }
    for s in 0..STEP_COUNT { p.steps[s].instruments[2] = true; }
    p
}
...
fn pattern_shuffle() -> Pattern {
    ...
    for &s in &[0, 6, 8] { p.steps[s].instruments[0] = true; }
    for &s in &[4, 12] { p.steps[s].instruments[1] = true; }
    for &s in &[0, 2, 4, 6, 8, 10, 12, 14] { p.steps[s].instruments[2] = true; }
    p
}
...
fn all_classics() -> Vec<Pattern> {
    vec![pattern_amen(), pattern_funky_drummer(), pattern_bossa_nova(), pattern_motorik(), pattern_shuffle()]
}

pub fn generate(style: Style, density: f32, variation: f32, rng: &mut impl FnMut() -> f32) -> Pattern {
    ...
    for step in 0..STEP_COUNT {
        for inst in 0..INSTRUMENT_COUNT {
            ...
            if rng() < var {
                let turn_on_prob = if was_active { 0.3 * (1.0 - density) } else { 0.3 * density };
