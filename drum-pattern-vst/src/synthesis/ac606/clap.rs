...
// [268] The measurement-fitted FIR/sinc tables below are data, not formulas:
// their long literals are the point. 767 lints of noise otherwise.
...
mod detail {
    ...
    impl CompressedSpectralCurve {
        ...
        pub fn configure(&mut self, source: &[f32; COLOR_TAP_COUNT], source_step: f32) {
            ...
            self.tap_count =
                ((COLOR_TAP_COUNT as f32 / step).ceil() as usize).clamp(1, COLOR_TAP_COUNT);
            if step == 1.0 {
            ...
            for tap in 0..self.tap_count {
                ...
                raw_energy += (self.coefficients[tap] as f64) * (self.coefficients[tap] as f64);
            }
    ...
    impl ClapCore {
        ...
        pub fn process(&mut self) -> CoreFrame {
            ...
            if terminal_age >= 0.0 {
                ...
                let mut terminal_color =
                    (1.0 - fast_mix) * color.handoff as f64 + fast_mix * color.fast as f64;
                let late_mix = raised_cosine_ramp(time, 0.110, 0.170);
                terminal_color = (1.0 - late_mix) * terminal_color + late_mix * color.late as f64;
    ...
    impl CoreUpsampler {
        ...
        fn ensure_generated(&mut self, final_index: i64, core: &mut ClapCore) {
            let final_idx = if final_index > 0 {
                final_index as u64
            } else {
                0
            };
...
impl AcClap {
    ...
    pub fn process(&mut self) -> f32 {
        ...
        let time = self.sample_index as f64 / self.sample_rate;
        let fade_position =
            ((time - 0.290) / (detail::VOICE_DURATION_SECONDS - 0.290)).clamp(0.0, 1.0);
