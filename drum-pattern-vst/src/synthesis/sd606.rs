...
use super::{dsp, sample_bank, saturation, settings::sd606::Sd606Settings, Voice, VoiceSettings};
...
pub struct Sd606Voice {
    settings: Sd606Settings,
    sample_rate: f32,
...
impl Sd606Voice {
    pub fn new(sample_rate: f32, settings: Sd606Settings) -> Self {
...
impl Voice for Sd606Voice {
    ...
    fn set_settings(&mut self, settings: VoiceSettings) {
        self.settings = Sd606Settings::from(settings);
...
mod tests {
    ...
    fn voice_with(settings: VoiceSettings) -> Sd606Voice {
        Sd606Voice::new(44100.0, Sd606Settings::from(settings))
    }

    #[test]
    fn produces_sound_on_trigger() {
        let mut voice = voice_with(VoiceSettings::sd606());
        voice.trigger();
        let mut peak = 0.0f32;
        for _ in 0..44100 {
    ...
    fn fixed_sample_mode_repeats_the_same_hit() {
        let mut settings = VoiceSettings::sd606();
        ...
        let capture = |voice: &mut Sd606Voice| -> Vec<f32> {
