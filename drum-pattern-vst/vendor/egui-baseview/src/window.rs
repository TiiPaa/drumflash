    ...
    U: 'static + Send,
{
    ...
    fn on_event(&mut self, _window: &mut Window, event: Event) -> EventStatus {
        if let Event::Mouse(mouse) = &event {
            let position = match mouse {
                baseview::MouseEvent::DragEntered { position, .. }
                | baseview::MouseEvent::DragMoved { position, .. }
                | baseview::MouseEvent::DragDropped { position, .. } => {
                    file_drop_position(_window, *position, self.pixels_per_point, self.egui_ctx.zoom_factor())
                }
                _ => None,
            };
            if let Some(status) = crate::file_drop::handle_event(
                &self.egui_ctx, &mut self.egui_input, mouse, position,
            ) {
...
// In baseview 9a0b42c's Windows backend the OLE POINTL is in SCREEN pixels,
// divided by the native scale, but not translated to client coordinates.
// Mouse movement is not delivered during an OS drag: use the event position,
// not egui's possibly stale last pointer position.
fn file_drop_position(
    window: &Window,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let raw_window_handle::RawWindowHandle::Win32(handle) = window.raw_window_handle() else {
            return None;
        };
        let mut point = POINT {
            x: (position.x * native_scale as f64).round() as i32,
            y: (position.y * native_scale as f64).round() as i32,
        };
        // SAFETY: baseview owns this live window; POINT is a writable local.
        if unsafe { ScreenToClient(handle.hwnd, &mut point) } == 0 {
            return None;
        }
        Some(pos2(point.x as f32, point.y as f32) / (native_scale * zoom))
    }
    #[cfg(not(target_os = "windows"))]
    {
        let _ = (window, native_scale);
        Some(pos2(position.x as f32, position.y as f32) / zoom)
    }
}
