...
use crate::params::persist::deserialize_field;
use crate::prelude::{
...
impl<P: Vst3Plugin> Wrapper<P> {
    ...
    unsafe fn load_state_from_stream(
        &self,
        ...
        self.log_state_diag(format_args!("{callback_name}: begin load"));
        let read_buffer = match self.read_state_stream(stream) {
            Some(read_buffer) => read_buffer,
            None => {
                self.log_state_diag(format_args!("{callback_name}: stream read failed"));
                return kResultFalse;
            }
        };

        self.log_state_diag(format_args!(
            "{callback_name}: read {} bytes",
            read_buffer.len()
        ));

        if allow_empty_state && read_buffer.is_empty() {
            self.log_state_diag(format_args!("{callback_name}: accepted empty state"));
            return kResultOk;
        }
        ...
        match state::deserialize_json(&read_buffer) {
            Some(mut state) => {
                self.log_state_diag(format_args!(
                    "{callback_name}: decoded state {}",
                    state_diag_summary(&state)
                ));
                if self.inner.set_state_inner(&mut state) {
                    self.log_current_params(callback_name, "after apply");
                    nih_trace!("Loaded state ({} bytes)", read_buffer.len());
                    kResultOk
                } else {
                    self.log_state_diag(format_args!("{callback_name}: set_state_inner failed"));
                    kResultFalse
                }
            }
            None => {
                self.log_state_diag(format_args!("{callback_name}: deserialize_json failed"));
                kResultFalse
            }
        }
    }

    unsafe fn read_state_stream(&self, stream: SharedVstPtr<dyn IBStream>) -> Option<Vec<u8>> {
        ...
        let mut eof_pos = 0;
        if stream.tell(&mut current_pos) != kResultOk
            || stream.seek(0, vst3_sys::base::kIBSeekEnd, &mut eof_pos) != kResultOk
            || stream.seek(
                current_pos,
    ...
    unsafe fn save_state_to_stream(
        &self,
        stream: SharedVstPtr<dyn IBStream>,
        callback_name: &str,
    ) -> tresult {
        ...
        self.log_current_params(callback_name, "before save");
        ...
        match serialized {
            Ok(serialized) => {
                ...
                self.log_state_diag(format_args!(
                    "{callback_name}: wrote {} bytes result={result} reported_written={num_bytes_written}",
                    serialized.len()
                ));
            ...
            Err(err) => {
                self.log_state_diag(format_args!("{callback_name}: serialize failed: {err:#}"));
...
fn state_diag_summary(state: &state::PluginState) -> String {
    let mut summary = format!(
        "version={} params={} fields={}",
        state.version,
        state.params.len(),
        state.fields.len()
    );
    ...
    for key in [
        ...
        if let Some(value) = state.params.get(key) {
            summary.push_str(&format!(" {key}={value:?}"));
        }
    }

    if let Some(serialized_pattern) = state.fields.get("pattern-v1") {
        let parsed_pattern: Result<[u8; 16], _> = deserialize_field(serialized_pattern);
        if let Ok(masks) = parsed_pattern {
...
impl<P: Vst3Plugin> IComponent for Wrapper<P> {
    ...
    unsafe fn set_state(&self, state: SharedVstPtr<dyn IBStream>) -> tresult {
        self.load_state_from_stream(state, false, "IComponent::set_state")
    }

    unsafe fn get_state(&self, state: SharedVstPtr<dyn IBStream>) -> tresult {
        self.save_state_to_stream(state, "IComponent::get_state")
    }
}
...
impl<P: Vst3Plugin> IEditController for Wrapper<P> {
    unsafe fn set_component_state(&self, state: SharedVstPtr<dyn IBStream>) -> tresult {
        // Some hosts restore the processor state through the edit controller for single-component
        // plugins. Loading the same state twice is harmless and keeps the controller-visible
        // parameter values in sync.
        self.load_state_from_stream(state, true, "IEditController::set_component_state")
    }

    unsafe fn set_state(&self, state: SharedVstPtr<dyn IBStream>) -> tresult {
        // Store the same state in the edit controller as in the component. This is redundant for
        // hosts that follow the split strictly, but it is required for hosts that persist only the
        // controller side for single-component VST3 plugins.
        self.load_state_from_stream(state, true, "IEditController::set_state")
    }

    unsafe fn get_state(&self, state: SharedVstPtr<dyn IBStream>) -> tresult {
        self.save_state_to_stream(state, "IEditController::get_state")
    }
...
impl<P: Vst3Plugin> IAudioProcessor for Wrapper<P> {
    unsafe fn set_bus_arrangements(
        ...
        if num_ins > 0 {
            check_null_ptr!(inputs);
        }
        if num_outs > 0 {
            check_null_ptr!(outputs);
        }
        ...
        let matching_layout = P::AUDIO_IO_LAYOUTS
            ...
            .find(|layout| {
                ...
                if num_ins as usize != num_layout_ins {
                    return false;
                }

                let min_layout_outs = if layout.main_output_channels.is_some() {
                    1
                } else {
                    0
                };
                if (num_outs as usize) < min_layout_outs || (num_outs as usize) > num_layout_outs {
                    return false;
                }
                ...
                for (aux_output_idx, channel_count) in layout
                    .aux_output_ports
                    .iter()
                    .take((num_outs as usize).saturating_sub(aux_output_start_idx))
                    .enumerate()
                {
    ...
    unsafe fn get_unit_by_bus(
        ...
        match (type_, dir) {
                    ...
                    && d == vst3_sys::vst::BusDirections::kOutput as i32 =>
            {
                ...
                if (0..main_busses + aux_busses).contains(&bus_index) {
                    *unit_id = kRootUnitId;
                    kResultOk
                } else {
                    kInvalidArgument
