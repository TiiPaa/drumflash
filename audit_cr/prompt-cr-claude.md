...
**LANGUE — IMPÉRATIF** : rédige tout le contenu du rapport (résumés, problèmes, suggestions, verdicts…) en **français soigné, AVEC les accents** (é, è, ê, à, â, ç, î, ô, ù, œ…). Ces consignes sont écrites sans accent pour des raisons techniques : **ne recopie PAS ce style** — ta sortie doit être parfaitement accentuée. Ne produis jamais de « français sans accents ».
...
**Parallelisation (si tu en as la capacite)** : si tu peux lancer des agents/sous-taches en parallele, tu es encourage a le faire pour auditer plusieurs axes SIMULTANEMENT (ex. un agent securite, un agent performance, un agent architecture/infra, un agent qualite/tests...). Le decoupage reste a TA discretion, mais **4 agents suffisent dans la quasi-totalite des cas** : au-dela, tu payes du temps de calcul et tu produis des doublons, pas de la couverture.

**Quota par agent** : chaque agent rend AU PLUS 8 findings — ses plus graves, deja fusionnes entre eux. Un agent qui en rend le double n'a pas fait son tri : il l'a delegue a l'etape de consolidation, qui n'est pas la pour ca.

**Consolidation (etape obligatoire)** : avant d'ecrire le fichier de sortie, relis l'ensemble des findings de tous les agents et applique, dans cet ordre :

1. **Fusionne** les findings qui decrivent le meme defaut vu sous deux angles (typiquement un agent securite et un agent qualite qui pointent la meme fonction) : un seul finding, qui garde la formulation la plus precise et cite tous les emplacements concernes.
2. **Ecarte** les findings sans scenario de defaillance concret, ou agrege-les (voir le critere d'admission dans la section Regles).
3. **Applique le budget global** defini dans la section Regles. Ce budget porte sur le rapport FINAL, pas sur chaque agent : la somme des quotas de tes agents depasse volontairement le budget final, c'est donc a toi de trancher — pas de rendre l'union.

La sortie est un rapport unique, coherent et deduplique — jamais une concatenation de listes d'agents.
...
**Remarques — priorite "hors_securite" (obligatoire meme si pas de faille) :**
...
- **modele_exact (OBLIGATOIRE)** : renseigne dans le champ "modele_exact" l'identifiant EXACT du modele que tu es (celui qui execute cet audit), tel que designe par ton fournisseur (ex: claude-fable-5, gpt-5-codex, gemini-2.5-pro). Si tu ne connais pas ton identifiant precis, indique le nom le plus precis que tu connais — ne laisse jamais ce champ vide.
- **Budget de findings (PLAFOND STRICT)** : au maximum 6 critiques, 10 importants, 8 suggestions, 6 hors_securite — soit 30 findings au grand maximum, toutes categories confondues. Ce n'est PAS un objectif a atteindre, c'est une limite a ne pas franchir. Un rapport au plafond est un rapport LOURD, pas un rapport complet.
- **Si tu depasses le budget** : ne tronque pas au hasard et ne rends pas tout quand meme. Classe par gravite reelle, garde les plus graves, et FUSIONNE les findings qui decrivent le meme defaut (meme cause racine, meme correctif) en un seul finding citant plusieurs fichiers et lignes. Un meme defaut present dans 12 fichiers = 1 finding, pas 12.
- **Plancher** : au moins 5 findings pour un projet non trivial. En dessous, tu n'as probablement pas assez cherche.
- **Critere d'admission** : un finding n'entre dans le rapport que si tu peux enoncer un scenario de defaillance concret (quelle entree ou quel etat produit quel dommage). Les remarques de style ou de gout depourvues d'un tel scenario ne sont pas des findings : regroupe-les en UN SEUL finding agrege de priorite "hors_securite", avec le nombre d'occurrences constatees.
...
```json
{
  "modele_exact": "Identifiant exact du modele qui a realise cet audit (ex: claude-fable-5, gpt-5-codex, gemini-2.5-pro)",
  "contexte": {
...
Les scores sont des nombres de 0 a 10. Langue: francais.
