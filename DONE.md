...
- [x] **[238] Flash de nom de lane bloqué** — l'adaptateur egui-baseview jetait le `repaint_delay` demandé pendant une frame déjà rendue : la frame d'extinction du flash n'était jamais schedulée, la plaque restait blanche jusqu'à la prochaine interaction (surtout les flashs 0,5 s Paste/Randomize/drop). Deuxième patch du vendor egui-baseview, documenté dans `FLASH-DRUM-PATCHES.md` (build 20260923-233248, à valider dans S1).
- [x] **[242]/[243] Rift : pitch live** — même défaut que One-Shot (`base_step` au trigger seulement), même fix dans `set_settings` (build 20260923-235211, compilé — **install + validation S1 en attente**, S1 ouvert au moment du build). Règle générale écrite dans `ADDING_AN_INSTRUMENT.md` §5 et la skill nouvel-instrument : paramètres continus live via `set_settings`, paramètres de structure du coup verrouillés au trigger.
