...
> Archive des taches terminees, sorties de `TODO.md`. Les titres reprennent ceux du
> TODO pour garder le contexte ; l'ordre du fichier d'origine est conserve.
...
### Phase 0 — Quick wins (build 20260924-124332)
- [x] [244] **CRITIQUE — Sauvegarde de pattern temps réel** : `lib.rs:2288` remplacer `refresh_snapshot()` par `mark_snapshot_dirty()` (la variante RT-safe existe déjà, pattern_bank.rs:1019-1024 ; le snapshot est reconstruit paresseusement dans `PersistentField::map()` hors callback).
- [x] [245] **Préallocation banque après restauration** : dans `PersistentField::set` (pattern_bank.rs:1055-1062), `reserve()` sur `plock_bytes` / `seq_plock_bytes` / `fusion_bytes` de chaque slot (sinon `capture()` réalloue ~165 Ko dans le callback à la première sauvegarde après réouverture d'un projet).
- [x] [246] **Renommage de preset invisible** : `presets.rs:376` — passer le `PresetKind` à `rename_preset` et construire le nom avec `kind.extension()` (`path.extension()` ne rend que `json`, le preset perd `.fdinst`/`.fdpat`/... et disparaît du navigateur). Corriger le test qui part d'une extension simple. Appelant : `ui/preset_browser.rs:396`.
- [x] [247] **Graphe Oh6smp affiche la mauvaise banque** : `ui/sound_editor.rs:3033-3040` — le `else` donne `ch606()` à Oh6smp (voice 24) alors que le DSP joue `oh606()` (2× plus long). Fix propre : fonction partagée `sampler_bank(voice_idx)` (13→bd606, 14→sd606, 15→ch606, 24→oh606) utilisée par UI et DSP + test d'équivalence.
- [x] [248] **Repli générateur qui paniquera** : `generator/mod.rs:159` — borner `_ => kind.drum_voice_index().min(INSTRUMENT_COUNT - 1)` (ou supprimer le `_` pour un match exhaustif) + test `every_kind_survives_generate` (chaque kind × 4 générateurs). Bloquant avant tout nouvel instrument ([56], [84], [95]).
- [x] [249] **Journal E:\tmp actif en release** : `vendor/nih-plug/src/wrapper/vst3/wrapper.rs:44` — rendre le journal d'état opt-in (`FLASH_DRUM_STATE_LOG`) et l'écrire dans `temp_dir()` ; ne re-sérialiser l'état (`log_current_params`) que si actif. Documenter le patch (rejoint [263]).
- [x] [250] **Verrou DLL avant install** : `build.ps1:121` — tester l'ouverture exclusive du DLL installé AVANT `Remove-Item` ; sortir code 2 sans rien toucher si verrouillé (S1 ouvert). (L'install atomique complète staging/rename est en phase 2, [255].)
- [x] [251] **Clippy + toolchain épinglée** : corriger les 2 constantes (`std::f32::consts::TAU` dans ac606/common.rs:7, `FRAC_1_SQRT_2` dans ac606/clap.rs:263) et ajouter `rust-toolchain.toml` (`channel = "1.94.0"`).
- [x] [252] **Compteurs voix/kinds dans les docs** : 27 voix / 25 kinds partout — CLAUDE.md (« 17 voices / 15 kinds »), ADDING_AN_INSTRUMENT.md §2, README racine + drum-pattern-vst/README (« 13 voix, Tom1-3, presets Rock/Funk/Disco »), docs/user-guide.md:21,26 (voix + Auto-Edit déplacé), docs/infrastructure.md (voix, hound en production, 477 tests, toolchain 1.94.0, CI existante).

## Nouvelles tâches — session 2026-09-23
- [x] [243] **Validations S1 du soir** : ① build **20260923-235211** (pitch Rift live via macro/automation) installé avec le build 20260924-124332 et validé dans S1 le 2026-09-24 ; ② fix du flash de nom de lane bloqué (build 20260923-233248 : Paste Lane / Randomize / drop WAV s'éteignent seuls, transport arrêté) validé dans S1 le 2026-09-24.
...
- [x] [243] **One-Shot, session du soir : Offset, enveloppes sampler, waveforms** — **Offset** (special 20, p-lockable) : fraction du fichier où la lecture **commence** dans les deux sens (en Reverse elle compte depuis la fin, le marqueur jaune reste à la position du knob). **Durées d'enveloppe en fractions de la région jouée** (× pitch) — sémantique sampler : 1.0 = tout le sample, fini les secondes absolues incompréhensibles sur sample court ; spéciaux renommés `_atk`/`_hld` (le garde-fou [182] exige une unité sur `_attack`/`_hold`). **Waveform en fond** des graphes Amp/Pitch/Filter (région jouée, inversée en Reverse, même axe temps que l'enveloppe) + graphe Offset section Sample (tête sautée grisée). **Attack amp réparée deux fois** : le cap 80 ms des voix drum était inaudible sur decay long, puis la capture du niveau au retrigger (anti-clic drum) tuait la rampe sur tout retrigger à queue vivante — un sampler repart de zéro à chaque coup (`trigger_hard`, le declick 3 ms couvre). **Ordre canonique [240] restauré** : le hoist par suffixe ne connaissait pas `_atk`/`_hld` (courbes remontées, Attack/Hold en queue) ; pitch env déclaré dans l'ordre canonique ; tests étendus. **Bouton « Random Offset on active cells » réservé à Rift** (le gate exige `_wander`). **Pitch live** : `base_step` + `region_secs` recalculés dans `set_settings` → l'automation/macro [242] ré-accorde la voix en continu au lieu d'attendre le prochain trigger (builds 20260923-205745 → 233248, **validé dans S1 le 2026-09-23**).
- [x] **[238] Flash de nom de lane bloqué** — l'adaptateur egui-baseview jetait le `repaint_delay` demandé pendant une frame déjà rendue : la frame d'extinction du flash n'était jamais schedulée, la plaque restait blanche jusqu'à la prochaine interaction (surtout les flashs 0,5 s Paste/Randomize/drop). Deuxième patch du vendor egui-baseview, documenté dans `FLASH-DRUM-PATCHES.md` (build 20260923-233248, **validé dans S1 le 2026-09-24**).
- [x] **[242]/[243] Rift : pitch live** — même défaut que One-Shot (`base_step` au trigger seulement), même fix dans `set_settings` (build 20260923-235211, **validé dans S1 le 2026-09-24**). Règle générale écrite dans `ADDING_AN_INSTRUMENT.md` §5 et la skill nouvel-instrument : paramètres continus live via `set_settings`, paramètres de structure du coup verrouillés au trigger.
...
- [x] [233] **Panneau Sons : sous-paramètres de la saturation décalés** - Amount, Mix et Output Gain ont leur libellé décalé de 14 px sous Saturation Type et sont grisés quand le type est None (tous instruments). Un premier essai généralisait la règle à tout le panneau (Grain sous Loop, enveloppe de filtre sous Filter Env, courbes, LFO, barre verticale) : rejeté par l'utilisateur (« rien n'est cohérent »), retiré au build 201521 ; seuls les trois sous-paramètres de saturation restent, sans barre. Retour utilisateur du 2026-09-21 (build 20260921-201521, **validé dans S1 le 2026-09-22**).

## Nouvelles tâches — session 2026-09-15
- [x] [230] **Settings : bouton « Auto-assign » (Outputs)** - une action à la demande, pas un mode (retour utilisateur : « un switch ça n'a pas de sens ») : au clic, chaque lane active est routée sur la sortie aux de son numéro (lane 1 → Out 1 … lane 14 → Out 14) par `assign_slot_output`, donc sortie du main mix comme une assignation manuelle ; rien n'est mémorisé, le sélecteur Aux Out reste libre. `assign_outputs_in_order` dans popups.rs. Demande du 2026-09-15 (builds 101709 → 102650, **validé dans S1 le 2026-09-22**).
## Nouvelles tâches — session 2026-09-13
- [x] [228] **Charger ses propres fichiers audio dans Rift** - tâche mise de côté au moment du plan (« embarqué d'abord, chargeable ensuite »). Demande : dialogue de fichier, décodage **hors thread audio**, chemin à persister dans la session et les presets, et le cas « fichier disparu » à gérer sans couper le son. Le décodeur RIFF de `sample_bank.rs` (f32 32 bits et PCM 16 bits) se réutilise tel quel. **Livré (builds 20260915-091757 -> 20260921-161353, **validé dans S1 le 2026-09-21**)** : menu Texture à 12 entrées (4 embarquées + `User 1..8`), ligne `File` sous le menu avec nom / « missing » en rouge / `Load...` (dialogue natif `rfd`, WAV toute profondeur, stéréo → mono, 60 s max, décodage `hound` sur le thread UI) / `Clear`. `TexturePool` par instance (`arc_swap`, lecture audio = un load atomique + clone d'Arc, génération précédente parquée côté UI pour ne jamais libérer sur le thread audio), `TextureSource` tenu par la voix le temps du coup, `resolve_texture` : emplacement vide ou fichier disparu → Noise, jamais le silence. Chemins persistés (`user-textures-v1`, décodés au restore sur le thread principal) et emportés par les presets d'instrument et de pattern (`user_texture`, `serde(default)`). Build 092641 : le dialogue de fichier tourne sur son propre thread (`TexturePick`, `poll_texture_pick`) - bloquant dans l'image egui, il ré-entrait la boucle de messages de baseview et faisait tomber S1 (panic = abort). Build 101709 : interrupteur **Stereo** sous la ligne File (std `Stereo` de Rift, famille Osc, rendu hors boucle des standards) : un fichier stéréo garde ses deux canaux (`TextureBank::right`), allumé = gauche à gauche / droite à droite, éteint = mixage mono au moment de la lecture ; Stereo Spread s'ajoute par-dessus ; grisé sur un fichier mono. Build 102650 : Stereo devient une ligne permanente de la section Texture, sous Stereo Spread (grisée tant que la texture courante n'a pas de canal droit), et le menu Texture affiche le fichier de chaque emplacement (« 3: kick.wav », « (missing) », « User 3 » si vide). **Build 134421, modèle refait sur retour utilisateur (« juste un wav par lane en plus des waves par défaut ») : un fichier custom PAR LANE (pool à 14 emplacements, un par slot ; persist `lane-textures-v1`), menu Texture à 5 entrées (4 d'usine + le fichier de la lane, affiché par son nom / « Custom (empty) » / « nom (missing) »), ligne File toujours visible, Load sélectionne Custom, fichier effacé si la lane change de type d'instrument (`reconcile_lane_textures`), partage du décodage entre lanes chargeant le même fichier (cache chemin+mtime+taille, `Weak`), copie de lane et presets emportent le fichier.** Build 161353 : Offset/Wander/Advance parcourent tout le fichier (la réserve de 4 decays mangeait un sample court, mesuré : rien ne bougeait), Reverse lit la durée de l'enveloppe à l'envers depuis l'offset, la texture suit la lane déplacée (`UserTextures::reorder` dans `apply_lane_reorder_move`), un fichier stéréo arrive avec Stereo allumé.
- [x] [221] **Rift — instrument FX/percussion par prélèvement dans une texture longue** — plan complet dans `~/.claude/plans/j-aimerais-ajouter-une-autre-mellow-wilkes.md`. Quatre textures embarquées de 6 s, une tranche prélevée à un **offset** réglable, sculptée par enveloppes ampli/filtre. L'offset étant un paramètre spécial continu, il est p-lockable par cellule et morphable sur une fusion **sans code dédié**.
  - [x] **Build 1 — le moteur nu** : `TextureBank` + décodeur WAV partagé, `RiftVoice` (lecture à position fractionnaire, fenêtre à fondu, filtre multi-mode `Biquad`, saturation), kind 23 / voix 25, catégorie Fx, note MIDI 58, rôle générateur emprunté à Perc1. Textures fabriquées par `tools/gen_textures.py` (graine fixe), **12 s chacune**. Dropdown Texture via le nouveau champ `options` du registre (générique, réutilisable). Decays ampli et filtre en **secondes absolues jusqu'à 1,5 s**. **Filtre aligné sur la convention du plugin** (formule de `buzz.rs` : l'enveloppe ouvre la coupure vers 20 kHz et elle y retombe ; Filter Env 0..1 ; libellé de coupure sans mention de type). **Window remplacé par Grain + interrupteur Loop** : Loop éteint, l'enveloppe décide seule de la durée ; Loop allumé, le grain se répète sous elle. +4,05 Mo sur la DLL (builds 20260913-115048 -> 151632, **validé dans S1 le 2026-09-14**).
  - [x] **Build 2 — le mouvement** : `dsp::Lfo` partagé (5 formes, dont sample-and-hold ; première brique LFO du projet), Pitch Fine, Pitch Env (±24 demi-tons) + Pitch Env Time, LFO de pitch et LFO de filtre partageant forme et politique de phase, et `Advance` (mode séquentiel, l'offset avance d'un cran par déclenchement). Spéciaux 17 à 26 (build 20260913-160405, **validé dans S1 le 2026-09-14**).
  - [x] **Build 3 — le caractère** : reverse (pas négatif), bitcrush + décimation, stéréo spread, et l'action générique **Spread / Scatter** qui écrit les p-locks d'un paramètre continu sur les 16 cellules d'une page. Spéciaux 27 à 30. Livré : `Stereo Spread` (OSC, seconde tête de lecture à droite, jusqu'à 0,5 s plus loin, même enveloppe), `Crush` + `Decimate` (SAT, quantification **relative au pic de la tranche** pour qu'aucun offset ne tombe dans le silence, décimateur sample-and-hold 1..64), et le menu **clic droit sur un slider** du panneau Sons : Spread up / Spread down / Scatter / Clear sur la page affichée, pour tout paramètre verrouillable de tout instrument (build 20260914-102711, **validé dans S1 le 2026-09-14**). Build 160256 : fin de coup sans clic (`cut()` arme le declick par canal ; mesuré −0,00097 → 0 en un échantillon avant). Build 164149 : son d'usine neutre (Filter 20 kHz, Filter Env 0, Offset 0,05) - le balayage de filtre d'usine et une zone rugueuse de la texture passaient pour une enveloppe de pitch après Default.
- [x] [227] **Advance : interrupteur + options repliables** - un switch on/off, et un menu repliable avec : reset au retour de chaque **pattern**, reset au retour de chaque **page**, et advance sur **chaque cellule** (active ou non) plutôt que sur chaque déclenchement. **Point dur, à traiter avant de coder** : le compteur vit aujourd'hui dans la voix, qui ne connaît ni le pattern, ni la page, ni les cellules inactives. Il faut le **déplacer dans `lib.rs`**, qui connaît les trois, et lui faire écrire l'offset dans les réglages déjà construits par `voice_settings_at_step` - la voix n'aurait alors plus besoin du paramètre Advance du tout. Le menu repliable n'existe pas encore dans le Sound Editor. **Livré (builds 20260914-124217 -> 154151, **validé dans S1 le 2026-09-14**)** : interrupteur `Advance` + slider `Advance Step` grisé quand éteint + pli `Advance options` (Reset on pattern loop / Reset on page change / Count every cell). Le compteur a quitté la voix : `lib.rs` compte par slot (`advance_hits`, reset sur wrap de pattern ou changement de page), le mode « chaque cellule » se déduit de la position sans compteur, et la voix reçoit l'index par `Voice::set_hit_index` avant chaque `trigger`. Options stockées dans 14 `IntParam` cachés `advmode_N` (bitfield ON=1 / RESET_PATTERN=2 / RESET_PAGE=4 / EVERY_CELL=8), persistés par l'hôte. Même build : section **Pitch Envelope** dédiée avec graphe (`ParamFamily::PitchEnv`), et tout paramètre 0/1 discret devient un interrupteur (Reverse, Loop). Build 144144 : les params `advmode_N` ne sont plus cachés (un param caché est lecture seule pour l'hôte, donc le bouton ne basculait pas pendant la lecture), et sections réorganisées : Texture / Pitch / Amp / Filter (Resonance sous la coupure, Filter Attack et Hold sous Filter Env) / Modulation / Distortion. Build 154151 : « Reset on page loop » (un wrap de pattern compte comme un retour de page), les deux resets s'excluent, Advance éteint referme les options, graphes alignés sur la première ligne de leur section, bouton « Random Offset on active cells » sous Offset (`PlockState::fill_steps`, `random_values`).
- [x] [229] **BUG REAPER : aucune saisie clavier possible** - le garde-fou du contournement clavier (`win_keyboard`, nih_plug_egui vendoré) vérifiait que le plugin est au premier plan en **remontant la chaîne des fenêtres parentes**. Vrai dans un éditeur ancré (Studio One), **jamais** dans une fenêtre de plugin flottante : la fenêtre de FX de REAPER est une fenêtre de premier niveau hors de cette chaîne, donc la remontée finissait à zéro et le focus n'était jamais posé. Journal à l'appui : `abort: plugin not in foreground chain` à chaque image, `current_focus=0x0`. Le garde-fou teste désormais le **processus propriétaire** de la fenêtre de premier plan, ce qui est la question qu'il voulait poser. Journal de diagnostic rendu activable en release via `FLASH_DRUM_KBD_LOG=1`. Deux causes de plus trouvées par la trace : la fenêtre de messages ne répondait pas à `WM_GETDLGCODE`, donc le dialogue `#32770` de REAPER avalait les WM_CHAR (deux lettres passaient puis plus rien) ; et l'espace lançait le transport - elle répond maintenant `DLGC_HASSETSEL` comme un vrai champ `Edit` (sans effet, REAPER ne consulte pas ce code). Puis l'espace : REAPER offre chaque touche via `IPlugView::onKeyDown` AVANT ses raccourcis, et quand le plugin répond « traitée » il ne poste plus le message Windows ; le plugin prend donc la touche ET la livre lui-même à egui par le canal `WM_APP` (`HOST_KEY_HANDLER`) (builds 20260913-192149 -> 20260914-094858, **validé dans S1 et REAPER le 2026-09-14**).
- [x] [226] **Reverse audio + affichage du granulateur** - `Reverse` lit la tranche à l'envers (pas négatif, bornes et rebouclage adaptés dans les deux sens). Le graphe de la texture montre en plus la **fenêtre de grain** quand Loop est allumé, avec des chevrons qui donnent le **sens de lecture** et disent qu'elle se répète (build 20260913-190518, à valider dans S1).
- [x] [225] **Graphe de la texture + sliders de même longueur partout** - la section Oscillator de Rift affiche la **forme d'onde de la texture**, la **position de lecture** (Offset), la **bande** où Wander peut la jeter et les **prochaines positions** d'Advance. Résumé min/max en 512 colonnes calculé une fois au décodage. Et dans le Sound Editor, **toutes les sections gardent la même largeur de sliders** (celle des sections à graphe) : les sections sans graphe étalaient leurs sliders sur toute la largeur, donc un même paramètre n'avait pas la même longueur d'une section à l'autre (build 20260913-185053, à valider dans S1).
- [x] [224] **Le double-clic ne remettait pas la valeur d'usine de l'instrument** - `param_default` ne lisait la table du registre que pour les samplers, SDrex et Rift ; les vingt autres retombaient sur un `VoiceSettings::default()` partagé. **Mesuré avant correction : 20 instruments sur 26 concernés, 17 sliders remis à une valeur hors de leur propre plage** (Tone du HiHat à 60 Hz au lieu de 8000, Filter Env du Tom 1 à 0 au lieu de 1, Decay du Clap à 0,5 s au lieu de 0,03 s). Chaque voix lit désormais sa propre table ; le bouton **Default** en profite aussi. Test de garde sur les 26 voix (build 20260913-162418, à valider dans S1).
- [x] [223] **Le graphe d'enveloppe de filtre s'arrêtait avant le bord** - il se réservait 15 % de marge là où le graphe d'ampli occupe toute la largeur ; marge supprimée, la courbe atterrit sur la ligne de coupure au repos au bord droit. Buzz et SDrex partagent ce graphe et en profitent (build 20260913-154758, à valider dans S1).
- [x] [222] **BUG Ext MIDI : aucune lane ne repondait aux notes entrantes** - le plugin **emettait** sur la note MIDI de chaque lane mais **ecoutait** sur la note d'usine du registre (`voice_idx_from_midi_note`). Une lane reaccordee ne repondait jamais, et une note partagee par plusieurs voix resolvait vers la premiere qui la declare - souvent Snare606 (40) ou OpenHiHat (46), retirees depuis [203]/[204] donc sur aucune lane : rien ne jouait. Nouveau `AtomicTrackLayout::slots_listening_to(note)` ; plusieurs lanes sur une meme note declenchent toutes. Fonction fautive supprimee, test de garde (build 20260913-153150, à valider dans S1).
## Nouvelles tâches — session 2026-09-11
- [x] [212] **Mettre une page en boucle** — pouvoir isoler une page de 16 pas et la faire tourner seule, indépendamment de la longueur du pattern. À étudier : interaction avec `pattern_length`, le mode Follow, le mode Song, le resync hôte (`sync_to_host`) et l'export MIDI. **Livré (build 20260914-162358, **validé dans S1 le 2026-09-14**)** : param `page_loop` (0 = off, 1-4), double-clic sur un bouton de page ou « Loop this page » dans son menu clic droit, anneau ambre sur la page bouclée. `Sequencer` : `set_page_loop`, `loop_bounds`, `fold_into_loop` (engagement immédiat, phase conservée dans la page, lanes recalées au saut), recalage des compteurs à chaque retour de page (`expected` vs `shifted_master`), tir anticipé aux bornes de page, `host_to_local` partagé avec le détecteur de seek. Ignoré en mode Song ; page partielle (Len 40, page 3 = 8 pas) bouclée telle quelle ; page au-delà du pattern = off et le param repasse à 0. Export MIDI inchangé.
- [x] [220] **Trois couleurs pour la grille : bleu, violet, vert clair** - l'ambre disparait. Le p-lock **sound** passe de l'ambre au **vert clair** `#b5ffe1`, le p-lock **sequenceur** garde le **violet** `#a855f7`, et la pastille marche **dans les deux sens** (elle signale toujours l'autre type). Les cellules etant des **bitmaps bakes**, l'atlas a ete re-teinte : les emplacements `seq` gardent les pixels violets, les emplacements `link` sont un remap teinte/saturation du violet vers le vert clair, calibre sur le mi-ton du pad pour garder le degrade et le sertissage skeuo (builds 20260913-093827 -> 20260913-105120, a valider dans S1).
- [x] [219] **Condition « And » + pastille violette** - second terme combine en ET (bits 16-23 du `u32` existant, aucun changement de format), `Not` inverse l'ensemble ; pastille violette en haut a droite des cellules a p-lock sequenceur quand la grille affiche le sound - **c'est la reponse a [214]** (build 20260912-103510, a valider dans S1).
- [x] [218] **Menu p-lock allege + « Not » sur les conditions** - clic droit ferme le menu ; ligne « Mode » retiree ; bouton **Not** qui inverse n'importe quelle condition (modificateur, grise sur `Always`) et remplace « Not 1st loop », retire de la liste. Drapeau range dans un bit haut du `u32` existant : aucun changement de format, les sessions anciennes jouent a l'identique (build 20260912-101156, a valider dans S1).
- [x] [217] **Clear Plock semblait ne rien faire** - le clic droit pointe le panneau sur la cellule en portee Step, et en portee Step toucher une rangee **recree** le p-lock ; le panneau restait pointe la apres le Clear. Effacer un p-lock relache desormais la selection du panneau (build 20260912-093224, a valider dans S1).
- [x] [216] **Retirer le bouton T + Clear Plock ferme le menu** - le bouton cumulait l'audition et la lampe temoin ; retire, lanes a deux pastilles. **La lampe restait allumee : bug de repaint**, l'extinction n'etait jamais programmee - corrige. L'indicateur MIDI est conserve sous forme de LED sur la plaque de nom. L'audition reste cablee cote audio sans declencheur (build 20260911-200326, a valider dans S1).
- [x] [215] **Simplifier le menu du p-lock** - « Create Plock » mene directement au panneau (plus de second ecran) ; « Edit In Panel » et « Paste Plock » retires du menu d'une cellule p-lockee, qui n'a plus que Copy et Clear sous la ligne Mode (build 20260911-191858, a valider dans S1).
- [x] [211] **P-lock : supprimer l'option « Snapshot Current Settings »** - retiree du menu de creation, le bouton restant devient « Create Plock ». Le format est intact : un snapshot enregistre avant se charge et se joue, et la ligne Mode le nomme toujours « Full Snapshot ». `SoundSettingsState` sort de la signature du menu et du popup (build 20260911-190921, a valider dans S1).
- [x] [213] **Choisir le type de p-lock dans le menu contextuel** - segmente `Sound | Sequencer` en tete du popup, local a la cellule (`PlockPopup::sequencer`), le mode de la grille n'est plus touche. Le Lane Editor suit le choix. **Limite : la couleur de la cellule suit toujours le mode de la grille, c'est [214]** (build 20260912-094023, a valider dans S1).
- [x] [214] **Affichage unique quand une cellule porte les deux types de p-lock** - resolu par [219] : pastille violette en haut a droite quand la grille affiche les p-locks sound.
...
### P1 — Quick wins
- [x] [210] **Le premier clic d'une fusion ne se voyait pas** - deux causes : l'etat etait efface des que MAJ remontait (il survit maintenant, Echap annule), et le marqueur etait invisible (`rgb(20,34,58)` contre une cellule vide `rgb(27,27,34)`). Remplissage tire vers l'accent, lisere 2 px, et un crochet `[` peint dans la cellule (`skeuo::fusion_start_bracket`). Apercu du span au survol propose, non retenu pour l'instant (build 20260909-164540, a valider dans S1).
- [x] **Notes du 2026-09-09 demandees en priorisation** - les cinq points transmis etaient **deja livres ou tranches**, d'ou l'absence de nouvelle tache : l'onglet sound qui s'ouvrait sur un seq plock = **[199]** (`ui/grid.rs`, un clic droit en mode Sequencer efface la selection de cellule) ; le bouton Default, demande deux fois = **[200]** (+ [200b] desarmement du « Sure? », [200c] sensible a la portee, [200d] bandeau d'actions) ; les boutons Store/Restore = **[201]** ; instruments + params par pattern = **[202]**, abandonne par l'utilisateur. Les lignes correspondantes ont ete retirees de `docs/notes/notes.txt` ; cette entree en garde la trace.
- [x] [208] **Instrument OH6smp** (charley ouvert multisample, `wav/OH.wav`) - kind 22 / voix 24, note 46 ; un seul moteur pour les deux charleys via `Ch606Voice::with_bank` ; onze listes `13 | 14 | 15` remplacees par `is_sampler()` ; role generateur 3 (build 20260909-144136, a valider dans S1).
...
- [x] [188] **HiHat decay : max 1,5 s** - fait dans `HIHAT_STD` (5 s -> 1,5 s) ; OpenHiHat garde ses 5 s (queue longue = charley ouvert), hats AC606 deja a 2 s ; aucun clamp DSP a aligner (build 20260909-113627, a valider dans S1).
- [x] [189] **Slider Saturation Amount plus progressif** - loi de reponse `exposant 1,5` sur les 24 definitions, portee par le registre (`SpecialParamDef::curve`, `sp_curved`, `SAT_AMOUNT_CURVE`) et par `TrackStyle::with_curve`. Choisi par mesure du THD (15/32/43 % en lineaire -> 7/23/38 %). Aucun son modifie : la loi deplace la poignee, pas la valeur (build 20260909-114633, a valider dans S1).
- [x] [207] **Compensation de gain de la saturation incoherente** - egalisation en **energie (RMS)** sur un signal de test au lieu d'une normalisation en un point unique. Derive du Valve +3,53 -> +0,58 dB, dispersion entre types a mi-course 3,16 -> 0,64 dB, pic du Valve 1,357 -> 0,899. Taux de distorsion inchanges : seul le niveau bouge. 3 gardes remplacent l'ancien test qui affirmait le defaut (build 20260909-150755, a valider dans S1).
- [x] [190] **Scrollbar du Sound Editor** - **partiellement fait.** La cause principale etait que la barre etait *invisible au repos* (defaut egui `floating` + `dormant_handle_opacity: 0.0`) : elle est desormais permanente, large de 10 px, avec le sillon `skeuo::groove_fill()` partage avec les sliders et une poignee FAINT/INK3/INK2, sans decalage de mise en page (build 20260909-115534).
  - **Le relief skeuo n'est PAS fait** : egui ne peint que deux rectangles plats, donc regler couleurs et largeur ne peut pas produire de creux ni de degrade. Retour utilisateur sur le build : « juste plus epaisse mais completement unie ». Aggrave par le fait qu'un contenu qui depasse a peine la hauteur visible donne une poignee occupant ~85 % de la barre, qui se lit alors comme un bloc.
  - **Suite possible, mise en attente le 2026-09-09** (« on reste comme ca pour l'instant ») : peindre la barre dans `skeuo.rs` (sillon + levre + ombre interne, poignee en degrade avec filet clair et stries) et **reprendre a egui le clic-glisser** - masquer sa barre, piloter l'offset via `vertical_scroll_offset`. Le risque est la : molette, clic dans le vide pour sauter d'une page, et contenu dont la hauteur change en cours de route, tous geres par egui aujourd'hui. Maquette comparative dans `ui-preview/scrollbar-c.png` (non versionne).

### P2 — Retraits d'instruments (décision utilisateur : retrait + migration)
- [x] [203] **Retirer Snare606** (remplace par SD6(AC)) - retire des pickers via `selectable()` (les variantes et la voix DSP restent : l'index est persiste), lanes sauvegardees migrees vers Sd6Ac dans `PersistentField::set`, kit 12 lanes adapte (build 20260909-142638, a valider dans S1).
- [x] [204] **Retirer OpenHiHat** - meme mecanique, migration vers HiHat ; la lane garde sa note MIDI, un nom par defaut suit le nouveau kind. Kit 12 lanes : second HiHat qui choke avec le premier. Rien a changer cote choke legacy ni generateur (`Oh6Ac` garde le role 3) (build 20260909-142638, a valider dans S1).
...
- [x] [191] **Refaire la representation graphique du link lane** - le filet de 2 px devient une **fleche retour** sur la seule lane esclave (la maitresse ne porte aucune marque) ; `skeuo::link_arrow`, 9 x 16 px dans l'espace deja libre entre poignee et plaque de nom, aucun decalage. Accolade, cellules ternies et nom en retrait ecartees apres maquettes (build 20260909-152610, a valider dans S1).
- [x] [191b] **Le deplacement de lane cassait les liens** - un lien etant positionnel : une esclave n'est plus deplacable (curseur d'interdiction + explication au survol), une maitresse emporte sa chaine (`chain_len` + `lane_move_order_block`, equivalente a l'ancienne permutation pour une lane seule sur les 196 couples), aucun depot au milieu d'une chaine (`snap_gap_out_of_chains`, applique aussi a l'indicateur). 4 tests (build 20260909-154202, a valider dans S1).
- [x] [192] **Virer l'OpenHiHat** - **doublon de [204]**, tranche et fait le 2026-09-09 : retrait des pickers + migration des lanes vers HiHat.

### P2/P3 — Routing des sorties (gros chantier, les deux points se touchent)
- [x] [193] **Assigner une output deconnecte le Main** - la capacite existait deja (interrupteur Main Mix par lane) ; ajoute le **couplage** : `Out N` baisse l'interrupteur, `No Aux` le remonte, et l'interrupteur reste libre pour du traitement parallele. Sessions existantes non reecrites (build 20260909-162500, a valider dans S1).
- [x] [194] **Router plusieurs tracks vers la meme output** - rien a faire cote DSP, la boucle aux additionnait deja ; seul le **vol** de sortie en UI bloquait. `assign_slot_output` n'exclut plus, le selecteur affiche `Out 3 - 2 lanes` (`lanes_on_output`, decompte total). **La regle d'exclusivite de [117] est levee** - rien n'en dependait (build 20260909-162500, a valider dans S1).

## Nouvelles tâches — session 2026-08-20

### Bugs
- [x] [187] **Le paramètre masqué par l'Attack redevient verrouillable** — dans `plock-v1`, l'Attack ajoutée tardivement au champ 18 (= `SPECIAL_FIELD_START + 4`) volait sa case au spécial d'indice 4, ignoré sans explication sur les 18 voix (Output Gain du Kick, Wet de SDrex…). Relogé sur le champ 45 (emplacement du spécial 31, qu'aucune voix ne déclare) : **aucun changement de format**, et le champ 12 du repli legacy du Clap reste intact. Les anciens snapshots (46 bits de masque, valeur 0 au champ 45) sont neutralisés par `sanitize_field_mask` dans `set_raw`, la nouvelle valeur de masque servant de marqueur de version. Infobulle de grisage réparée au passage (elle était accrochée à un label vide, donc insurvolable) (build 20260821-160115, validé S1 2026-08-21).
- [x] [186] **CRASH de l'hôte au rechargement du plugin** — session pleine → fermeture → session vierge + ajout de Flash Drum = crash S1, reproductible. Cause : la classe de fenêtre du pont clavier (`vendor/nih-plug/nih_plug_egui/src/editor.rs`) était enregistrée sous un nom **fixe** avec le `HINSTANCE` de l'**EXE hôte**, jamais désenregistrée, jamais accompagnée d'un `DestroyWindow` ni d'une restauration du `WndProc` de l'hôte — donc elle survivait au déchargement de la DLL avec un pointeur de fonction pendouillant, que le chargement suivant réutilisait aveuglément (`RegisterClassW` échouait, le code traitait ça comme un succès). Violation DEP en exécution, confirmée par trois minidumps. Fix : nom de classe **unique par chargement** + `HINSTANCE` de notre DLL + `uninstall()` complet depuis `Drop` (WndProc restauré, fenêtre détruite, classe désenregistrée) + comptage de références pour le multi-instance (build 20260821-120829, validé S1 2026-08-21).
- [x] [185] **L'algo p-locké inaudible** — le prologue de `process()` repoussait l'algo global sur chaque voix à chaque buffer sans condition, écrasant l'algo appliqué au déclenchement depuis le p-lock : le coup gardait son algo quelques millisecondes puis la queue repassait au global. Fix : propagation conditionnelle au changement (`last_algos`), avec re-poussée forcée à la restauration d'état et au changement de kind. Test qui prouve que la repoussée en pleine queue est audible (build 20260821-111726, validé S1 2026-08-21).
- [x] [184] **Refondre l'édition des p-locks sound dans l'onglet Sound** — **phase 0 FAITE et validée S1** (build 20260821-101410 : `src/param_id.rs` + accesseurs `VoiceSettings::get/set` et `InstrumentSettingsState::get/set`, 3 des 6 mappings dupliqués supprimés, `PlockState::clear_field`, `StandardField::ALL`, 4 tests). **phase 1a FAITE** (couche `ParamSource` composée + `AlgoSink` + 6 tests, commit `7cbcfa3`), **phase 1b FAITE** (panneau branché sur `GlobalSource`, graphes inclus, build 20260821-104025, validé S1 2026-08-21). **phase 2 FAITE** (sélection par clic droit, résolveur pur + 6 tests, badge et filet d'accent, source polymorphe — builds 20260821-124406 + 141012, validé S1 2026-08-21 : 9/10, verrou Follow levé au passage). **phase 2c FAITE** (gouttière de 14 px réservée dans les deux portées, barre d'accent cliquable pour rendre un paramètre à la lane, rangées non verrouillables grisées avec motif, collision d'ids egui panneau/popup évitée via `salt()` — build 20260821-141959, validé S1 2026-08-21). **phase 3 FAITE** (segmenté `Start | End`, table 2×2 onglet × direction qui fait fonctionner les cibles héritées sans migration, plafond de 4 cibles visible, écritures groupées, 7 tests — builds 20260821-171425 → 20260822-112337, validé S1 2026-08-22). **phase 4 FAITE** (troisième onglet `Step` pour le p-lock d'une cellule fusionnée + ~930 lignes supprimées, `ui/plock.rs` 1257 → 603 — builds 20260822-115906 + 20260823-091039, validé S1 2026-08-23). **Refonte terminée.** Suite optionnelle : remonter les widgets des paramètres discrets dans le registre (supprime ~87 lignes de devinette par sous-chaîne de libellé dans le panneau). — remplacer le menu contextuel par le panneau de droite, pour n'avoir plus qu'UNE implémentation de rangées de paramètres au lieu de trois (panneau global `sound_editor.rs` ~745 l. / menu p-lock `ui/plock.rs:21-448` / menu morph `ui/plock.rs:497-820`). Clé : nouveau `src/ui/param_target.rs` avec `ParamId` (identité canonique, absorbe les 5 mappings `StandardField` dupliqués) + `ParamCtx` (portée global / p-lock du pas / extrémité de morph, écritures groupées). Sélection par clic droit, switch **Début/Fin** pour le morphing, ↺ par rangée, les 32 spéciaux verrouillables. Livraison en 4 phases testables. **Plan complet** : `~/.claude/plans/j-aimerais-ajouter-une-autre-mellow-wilkes.md`.
...
### Features moyennes (P2)
- [x] [144] **Snare : améliorer l'algo** — pas assez de corps (enrichir la synthèse : body oscillator, tuning, noise blend). — **Marque fait le 2026-09-13 (decision utilisateur).**
...
- [x] [27] Generation IA de patterns par style (rock, techno, rap, jazz, reggae, metal, funk, latin, disco, trap) — **Marque fait le 2026-09-13 (decision utilisateur).**
...
- [x] [41] Émulation circuit-exact TR-606 (WDF, modèle non-linéaire VCA, oversampling) — vs grey-box actuelle — **Marque fait le 2026-09-13 (decision utilisateur).**
